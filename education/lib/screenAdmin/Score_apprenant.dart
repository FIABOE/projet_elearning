...
class ScoreApprenant extends StatefulWidget {
   final int apprenantId;
  const ScoreApprenant({super.key , required this.apprenantId});

  @override
  _ScoreApprenantState createState() => _ScoreApprenantState();
}

class _ScoreApprenantState extends State<ScoreApprenant> {
...
Future<void> fetchGlobalData() async {
  ...
  try {
    ...
    final response = await http.get(Uri.parse('$BASE_URL/api/get-global_Score/apprenants/${widget.apprenantId}'));

    if (response.statusCode == 200) {
      final Map<String, dynamic> responseData = json.decode(response.body);

      // Assurez-vous que les valeurs ne sont pas nulles avant de les affecter
      double faible = double.parse(responseData['faible'].toString()) ?? 0.0;
      double fort = double.parse(responseData['fort'].toString()) ?? 0.0;

      setState(() {
        globalValues = [faible, fort];
        loadingGlobalData = false; // Désactivez l'indicateur de chargement
      });
    } else {
  ...
  try {
    ...
    final response = await http.get(Uri.parse('$BASE_URL/api/get-Niv_Score/apprenants/${widget.apprenantId}'));
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        loadingNiveauData = false; // Désactivez l'indicateur de chargement
      });
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  const Text('Fort: ', style: TextStyle(fontSize: 18, color: Colors.black)),
                  Text(
                    '${data['fort']}',
                    style: const TextStyle(
                      fontSize: 18,
...
Widget _buildAverageChart() {
  if (loadingGlobalData) {
    ...
    if (globalValues.isEmpty) {
      ...
      final List<double> averageValues = globalValues;
          ...
          children: [
            ...
            const Row(
              ...
              children: [
                ...
                SizedBox(width: 16),
                LegendItem(color: Color(0xFFF28435), label: 'Fort'),
              ],
            ),
          ],
        ),
