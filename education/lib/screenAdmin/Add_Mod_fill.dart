...
class Add_ModFiliere extends StatefulWidget {
  const Add_ModFiliere({super.key});

  @override
  _Add_ModFiliereState createState() => _Add_ModFiliereState();
}

class _Add_ModFiliereState extends State<Add_ModFiliere> {
  ...
  Future<void> ajouterFiliere(String filiere) async {
    ...
    const String apiUrl = '$BASE_URL:8000/api/filieres';
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // Après avoir ajouté la filière, appelez fetchData pour mettre à jour les totaux
        await fetchData();
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Cet Module existe déjà.',
              style: TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.bold,
                color: Colors.white,
          ...
          TextButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const ListFilieree(),
                ),
              );
            },
            child: const Row(
  ...
  return TextField(
    ...
    onChanged: (text) {
      setState(() {
        isValiderButtonEnabled = text.isNotEmpty && !isSubmitting;
