...
class Objectif_ModStatisticsPage extends StatefulWidget {
  const Objectif_ModStatisticsPage({super.key});

  @override
  _Objectif_ModStatisticsPageState createState() => _Objectif_ModStatisticsPageState();
}

class _Objectif_ModStatisticsPageState extends State<Objectif_ModStatisticsPage> {
  late List<UserData> chartData;

  @override
  void initState() {
    super.initState();
    fetchDataAndSetState(ApiManager.fetchObjectifCountByDay);
  }
            ...
            TabBar(
              ...
              onTap: (index) {
                if (index == 0) {
                  ...
                  fetchDataAndSetState(ApiManager.fetchObjectifCountByWeek);
                } else if (index == 2) {
                  fetchDataAndSetState(ApiManager.fetchObjectifCountByMonth);
                }
              },
            ),
            Expanded(
              child: TabBarView(
                children: [
                  buildChart('Jour', ApiManager.fetchObjectifCountByDay, 'Nombre d\'ajout par jour'),
                  buildChart('Semaine', ApiManager.fetchObjectifCountByWeek, 'Nombre d\'ajout par semaine'),
                  buildChart('Mois', ApiManager.fetchObjectifCountByMonth, 'Nombre d\'ajout par mois'),
                ],
              ),
            ),
          ],
...
class ApiManager {
  static Future<List<UserData>> fetchObjectifCountByDay() async {
    return _fetchData('objectif-stats');
  }

  static Future<List<UserData>> fetchObjectifCountByWeek() async {
    return _fetchData('objectif-week');
  }

  static Future<List<UserData>> fetchObjectifCountByMonth() async {
    return _fetchData('objectif-month');
  }
...
class UserData {
  ...
  factory UserData.fromJson(Map<String, dynamic> json) {
    return UserData(
      ...
      json['objectif_count'] ?? json['count'],
    );
  }
